      ...
      case Source is
         ...
         when SYSCLK_SRC_PLL =>
            declare
               ...
               PLL_HSE_PREDIV : constant UInt32 :=
                  UInt32 (RCC_Periph.CFGR2.PREDIV);
               PLLOUT         : UInt32;

            begin
               case PLL_Source'Val (RCC_Periph.CFGR.PLLSRC) is
                  when PLL_SRC_HSE_DIVPREDIV =>
                     if PLL_HSE_PREDIV < 16 then
                        PLLOUT :=
                           (Param.HSE_Clock / (PLL_HSE_PREDIV + 1)) * PLLMUL;
                     else
                        PLLOUT :=
                           (Param.HSE_Clock / (PLL_HSE_PREDIV)) * PLLMUL;
                     end if;
                  --  when PLL_SRC_HSI =>
                  when others =>
